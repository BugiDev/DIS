...
jQuery.fn.visualize = function(options, container){
	return jQuery(this).each(function(){
		...
		var parseLabels = function(direction){
			...
			if(direction == 'x'){
				self.find('thead tr').each(function(i){
					jQuery(this).find('th').each(function(j){
						if(!labels[j]) {
							labels[j] = [];
						}
						labels[j][i] = jQuery(this).text()
					})
				});
			}
			else {
		...
		if(o.parseDirection == 'x'){
			...
			for(var i=0; i<cols; i++){
				...
				self.find('tbody tr').each(function(j){
					dataGroups[i].points.push( {
						value:  jQuery(this).find('td').eq(i).text()*1,
						elem: this,
						tableCords: [i,j]
					} );
				});
		...
		jQuery(dataGroups).each(function(i,row){
			var count = 0;
			jQuery.each(row.points,function(j,point){
				allItems.push(point);
				count += point.value;
			});
		...
		jQuery.each(tableData.xAllLabels,function(i,labels) {
		...
		if(jQuery.isFunction(o.xLabelParser)) {
			
			var xTopValue = null;
			var xBottomValue = null;

			jQuery.each(xLabels,function(i,label) {
		...
		// populate some data
		jQuery.each(dataGroups,function(i,row){
			...
			charts.line = {
				...
				setup: function(area){
					...
					jQuery.each(yLabels, function(i){
						...
						var thisLi = jQuery('<li><span>'+value+'</span></li>')
							.css('bottom', posB);
				...
				draw: function(area) {
					...
					jQuery.each(dataGroups,function(h){
						...
						ctx.lineJoin = 'round';
						jQuery.each(this.points, function(g){
		...
		(function(){
			...
			charts.bar = {
				setup:function(){
					...
					jQuery.each(bottomLabels, function(i){
					...
					jQuery.each(leftLabels, function(i){
						var thisLi = jQuery('<li><span>'+this+'</span></li>').prependTo(ylabelsUL);
		...
		var canvasContain = (container || jQuery('<div '+(o.chartId?'id="'+o.chartId+'" ':'')+'class="visualize '+o.chartClass+'" role="img" aria-label="Chart representing data from the table: '+ title +'" />'))
			.height(o.height)
			.width(o.width);
		
		var scroller = jQuery('<div class="visualize-scroller"></div>')
		...
		if(o.appendKey){
			...
			jQuery.each(yAllLabels, function(i,label){
				jQuery('<li><span class="visualize-key-color" style="background: '+dataGroups[i].color+'"></span><span class="visualize-key-label">'+ label +'</span></li>')
					.appendTo(newKey);
			});
		...
		if(o.interaction) {
			...
			var tracker = jQuery('<div class="visualize-interaction-tracker"/>')
				.css({
					'height': o.height + 'px',
					'width': o.width + 'px',
					'position':'relative',
			...
			tracker.mousemove(function(e){
				...
				jQuery.each(charts[o.type].interactionPoints,function(i,current){
		...
		// init plugins
		jQuery.each(jQuery.visualizePlugins,function(i,plugin){
