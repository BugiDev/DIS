...
(function(jQuery){
	jQuery.visualizePlugins.push(function visualizeTooltip(options,tableData) {
		//configuration
		var o = jQuery.extend({
			tooltip: false,
		...
		var tooltip = jQuery('<div class="'+o.tooltipclass+'"/>').css({
				position:'absolute',
				display:'none',
				zIndex:90
			})
		...
		self.bind('vizualizeOver',function visualizeTooltipOver(e,data){
			...
			if(o.tooltipalign == 'left' || ( o.tooltipalign=='auto' && x-scroller.scrollLeft()<=scrollerW/2 ) ) {
				if(jQuery.browser.msie && (jQuery.browser.version == 7 || jQuery.browser.version == 6) ) {usescroll=false;} else {usescroll=true;}
				...
				if(jQuery.browser.msie && jQuery.browser.version == 7) {usescroll=false;} else {usescroll=true;}
